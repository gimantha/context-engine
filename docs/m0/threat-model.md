...
| T12 | Staged object is replayed or substituted | Ingest unintended content | Scoped expiry, content hash, type and size verification | M3 uploads are scoped to a source, size and type limited, hashed, expire, and must match the event before a job exists (`test_sources_api.py`) | Staged-object cleanup in M7 / Ingestion owner |
...
| T17 | Denial reveals a restricted title or existence | Enumeration | Stable generic error codes/messages | Denied example and adapter translation; M2 returns not-found for invisible resources and a fixed access-denied message otherwise (`test_api.py`) | Query and evidence routes in M5 / API owner |
