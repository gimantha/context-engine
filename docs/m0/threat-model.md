...
| T15 | Raw graph/Cypher surface bypasses policy | Bulk disclosure or mutation | No raw graph API/MCP tool; provider service is private | OpenAPI/MCP contracts and leak lint; M4 pins the provider's retriever with automatic routing off (`test_cognee_adapter.py`) | Dependency upgrades / Security owner |
